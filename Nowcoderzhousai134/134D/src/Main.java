...
//    private static final long MOD = (long) (998244353);
    ...
    private static void solve() throws IOException {
        ...
        for (int i1 = 1; i1 < n; i1++) {
            long currentVal = arr[i1];
            while (!dq.isEmpty() && (Math.abs(currentVal - map.firstKey()) > 1 || Math.abs(currentVal - map.lastKey()) > 1)) {
                cnt += dq.size();
                long removed = dq.pollFirst();
            ...
            dq.addLast(currentVal);
            map.put(currentVal, map.getOrDefault(currentVal, 0) + 1);
        }

        long size = dq.size();
        sc.println((long) (size + 1) * (long) size / 2 + (long) cnt);
    }

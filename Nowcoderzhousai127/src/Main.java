...
//    static final int mod = (int) (998244353);
    ...
    private static void solve() throws IOException {
        ...
        while(true){
            if(i>=str.length()||str.charAt(i)-'0'>=5)break;
            else i++;
        }

        if(i>=str.length()){
            sc.println(str.substring(0,str.length()-1)+""+0);
            return;
        }

        while(true){
            if(i<str.length()&&str.charAt(i)=='9'){
                i++;
            }else break;
        }
        ...
        BigInteger add = new BigInteger(substring).add(BigInteger.ONE);

//        sc.println(i);

        StringBuilder sb = new StringBuilder(add.toString());
